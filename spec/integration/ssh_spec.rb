...
describe GasSsh::Ssh do
  ...
  describe "SSH key file handling..." do
    ...
    before :all do
      GasSsh::Prompter.stub!(:user_wants_to_delete_all_ssh_data?).and_return("l")  # only delete's local keys
    end

    after :all do
      GasSsh::Prompter.unstub!(:user_wants_to_delete_all_ssh_data?)
    end

    describe "Detecting when files are missing..." do
      it "should detect when an id_rsa isn't in the .gas directory" do
        GasSsh::Ssh.corresponding_rsa_files_exist?(@uid).should be_false
      end
    end
    ...
    describe "Detecting when files exist..." do
      ...
      it 'should detect when an id_rsa is already in the .gas directory' do
        GasSsh::Ssh.corresponding_rsa_files_exist?(@uid).should be_true
      end
    end
    ...
    describe "File System Changes..." do
      ...
      it 'should create ssh keys in .gas && Gas.remove should be able to remove those files' do
        ...
        GasSsh::Ssh.stub!(:upload_public_key_to_github).and_return(false)
        ...
        lambda do
          GasSsh.delete(@nickname)
        end.should change{count_of_files_in(GAS_DIRECTORY)}.by(-2)

        STDIN.unstub!(:gets)
        GasSsh::Ssh.unstub!(:upload_public_key_to_github)
      end
      ...
      describe 'For the ssh directory...' do
        ...
        it "shouldn't overwrite an existing key in ~/.ssh that isn't backed up in .gas and the user aborts" do
          ...
          GasSsh::Prompter.stub!(:user_wants_to_overwrite_existing_rsa_key?).and_return(false)
          GasSsh.use @nickname2
          GasSsh::Prompter.unstub!(:user_wants_to_overwrite_existing_rsa_key?)
        ...
        it "should overwrite an existing, unbacked-up key in ~/.ssh if user wants" do
          ...
          id_rsa, id_rsa_pub = plant_bogus_rsa_keys_in_ssh_directory
          #  3)  Switch to that user
          GasSsh::Prompter.stub!(:user_wants_to_overwrite_existing_rsa_key?).and_return(true)
          GasSsh.use @nickname2
          GasSsh::Prompter.unstub!(:user_wants_to_overwrite_existing_rsa_key?)
          #  4)  The .ssh directory should not be changed
          File.open(SSH_DIRECTORY + "/id_rsa", "r") do |f|
        ...
        it "If there's a key in ~/.ssh that's backed up in .gas" do
          ...
          rsa, rsa_pub = GasSsh::Ssh.get_associated_rsa_key(@nickname)
          rsa2, rsa2_pub = GasSsh::Ssh.get_associated_rsa_key(@nickname2)
          # 2)
          GasSsh.use @nickname2
          File.open(SSH_DIRECTORY + "/id_rsa.pub", "r") do |f|
        ...
        it 'should be able to copy ssh keys in the ssh' do
          ...
          GasSsh::Ssh.use_current_rsa_files_for_this_user(@nickname)
        ...
        it "should have a UTILITY for deleting rsa files of user" do
          lambda do
            GasSsh::Ssh.delete_associated_local_keys!(@nickname2)
          end.should change{count_of_files_in(GAS_DIRECTORY)}.by(-2)
        end
      end
  ...
  describe "Networking stuff..." do
    before :all do
      ...
      GasSsh.delete(@nickname)
      GasSsh::Ssh.stub!(:get_username_and_password_and_authenticate).and_return(@credentials)

      VCR.use_cassette('instantiate_github_speaker', :record => :new_episodes) do
        @github_speaker = GasSsh::GithubSpeaker.new(@user, @username, @password)
      end
    end

    after :all do
      GasSsh.delete(@nickname)
      GasSsh::Ssh.unstub!(:get_username_and_password_and_authenticate)
    end
    ...
    describe "Should remove and insert keys into github" do
      it 'UTILITY:  should insert a new key into github and conversly remove that key' do
        ...
        VCR.use_cassette('key_installation_routine-Add_key', :record => :new_episodes) do
          GasSsh::Ssh.key_installation_routine!(@user, @sample_rsa, @github_speaker)
        end
    ...
    it "should add ssh keys to github when user is created, and delete them when destroyed" do
      # yes, delete all
      GasSsh::Prompter.stub!(:user_wants_to_delete_all_ssh_data?).and_return("a")  # all keys, local and github
      # create new user and use ssh handling
      GasSsh::Prompter.stub!(:user_wants_gas_to_handle_rsa_keys?).and_return(true)
      GasSsh::Prompter.stub!(:user_wants_to_use_key_already_in_ssh?).and_return(false)
      GasSsh::Prompter.stub!(:user_wants_to_install_key_to_github?).and_return(true)

      VCR.use_cassette('add-on-crteation-delete-on-deletion', :record => :new_episodes) do
        lambda do
          GasSsh.add(@nickname,@name,@email, @github_speaker)
        end.should change{get_keys(@username, @password).length}.by(1)

        lambda do
          GasSsh::Ssh.stub!(:get_nils).and_return({ :username => @username, :password => @password })
          GasSsh.delete(@nickname)
          GasSsh::Ssh.unstub!(:get_nils)
        end.should change{get_keys(@username, @password).length}.by(-1)
      end

      GasSsh::Prompter.unstub!(:user_wants_to_delete_all_ssh_data?)
      GasSsh::Prompter.unstub!(:user_wants_gas_to_handle_rsa_keys?)
      GasSsh::Prompter.unstub!(:user_wants_to_use_key_already_in_ssh?)
      GasSsh::Prompter.unstub!(:user_wants_to_install_key_to_github?)
    end

    it "Gas.Delete should be able to remove the id_rsa from .gas" do
      GasSsh::Prompter.stub!(:user_wants_to_delete_all_ssh_data?).and_return("a")
      GasSsh::Prompter.stub!(:user_wants_gas_to_handle_rsa_keys?).and_return(true)
      GasSsh::Prompter.stub!(:user_wants_to_use_key_already_in_ssh?).and_return(false)
      GasSsh::Prompter.stub!(:user_wants_to_install_key_to_github?).and_return(true)
    ...
    it 'Gas.ssh(nickname) should be able to add ssh support to a legacy user or an opt-out' do
      GasSsh::Prompter.stub!(:user_wants_gas_to_handle_rsa_keys?).and_return(false)
      GasSsh.add(@nickname,@name,@email)
      GasSsh::Prompter.unstub!(:user_wants_gas_to_handle_rsa_keys?)

      GasSsh::Prompter.stub!(:user_wants_gas_to_handle_rsa_keys?).and_return(true)
      GasSsh::Ssh.stub!(:upload_public_key_to_github)

      lambda do
        GasSsh.ssh(@nickname)
      end.should change{count_of_files_in(GAS_DIRECTORY)}.by(2)

      GasSsh::Ssh.delete_associated_local_keys!(@nickname)
