...
def create_user_no_git(nickname, name, email)
    ...
    GasSsh::Prompter.stub!(:user_wants_to_install_key_to_github?).and_return(false)

    GasSsh.add(nickname,name,email)

    GasSsh::Prompter.unstub!(:user_wants_gas_to_handle_rsa_keys?)
    #GasSsh::Ssh.unstub!(:user_wants_to_use_key_already_in_ssh?)
...
def remove_key_from_github_account(username, password, rsa)
  # get all keys
  keys = GasSsh::Ssh.get_keys(username, password)
  # loop through arrays checking against 'key'
  keys.each do |key|
      if key["key"] == rsa
        return GasSsh::Ssh.remove_key_by_id!(username, password, key["id"])
      end
  end
...
def delete_all_keys_in_github_account!(github_speaker)
  VCR.use_cassette('delete_all_keys_in_github_account', :record => :new_episodes) do
    github_speaker.keys.each do |key|
      GasSsh::GithubSpeaker.publicize_methods do
