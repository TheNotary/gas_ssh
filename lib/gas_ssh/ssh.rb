module GasSsh
  class Ssh
    ...
    def self.setup_ssh_keys(user)
      ...
      if GasSsh::Prompter.user_wants_gas_to_handle_rsa_keys?
        if corresponding_rsa_files_exist?(@uid) and GasSsh::Prompter.user_wants_to_use_key_already_in_gas?(@uid)
          return true  # We don't need to do anything because the .gas directory is already setup
        elsif !corresponding_rsa_files_exist?(@uid) and ssh_dir_contains_rsa? and GasSsh::Prompter.user_wants_to_use_key_already_in_ssh?   #  Check ~/.ssh for a current id_rsa file, if yes, "Do you want to use the current id_rsa file to be used as your key?"
        ...
        if corresponding_rsa_files_exist?(@uid) #in ~/.gas/
          delete_associated_local_keys!(@uid) if GasSsh::Prompter.user_wants_to_remove_the_keys_that_already_exist_for_this_user?(@uid)
        end
      end
      ...
      if corresponding_rsa_files_exist?
        if ssh_dir_contains_rsa?
          if current_key_already_backed_up?
            ...
            if GasSsh::Prompter.user_wants_to_overwrite_existing_rsa_key?
              write_to_ssh_dir!
            else
    ...
    def self.upload_public_key_to_github(user, github_speaker = nil)
      if GasSsh::Prompter.user_wants_to_install_key_to_github?
    ...
    def self.delete(nickname)
      ...
      case GasSsh::Prompter.user_wants_to_delete_all_ssh_data?
      when "a"
